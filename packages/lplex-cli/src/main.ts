...
import {
  ...
  type Event,
  type Filter,
} from "@sixfathoms/lplex";
import { discover } from "./discover.js";
import { type DeviceMap, formatFrame, printDeviceTable } from "./display.js";
...
const reconnectDelay =
  Number.parseFloat(values["reconnect-delay"] ?? "2") * 1000;
const ackInterval = Number.parseFloat(values["ack-interval"] ?? "5") * 1000;
...
async function runEphemeral(client: Client, devices: DeviceMap): Promise<void> {
...
async function runBuffered(client: Client, devices: DeviceMap): Promise<void> {
  const session = await client.createSession(
    {
      clientId,
      bufferTimeout: bufferTimeout as string,
